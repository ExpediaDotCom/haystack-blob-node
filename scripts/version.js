...
const moduleName = 'haystack-blobs';
...
pkginfo(opts, (error, data) => {
    ...
    if (!releaseVersion) {
        throw new Error('No git tag commit is found, fail to release the "haystack-blobs" module');
    }
    if (semver.lte(releaseVersion, currentVersion)) {
        throw new Error(`Current haystack-blobs on npm registry has greater than or equal new release version ${releaseVersion}. Check your git tag version`);
    }
